...
namespace DB.Repositories
{
    public class ContentRepository : RepositoryBase<ContentManagement, ContentManagementDTO>, IContentRepository
    {
        ...
        public ContentRepository(CSADbContext context, IMapper mapper,IHttpContextAccessor httpContextAccessor) : base(context, mapper, httpContextAccessor) { }


        public async Task<IEnumerable<ContentManagementDTO>> GetAllContentsAsync()
        {
            var residents = await _context.ContentManagement.ToListAsync();
        ...
        {
            var Content = await _context.ContentManagement.Where(x => x.Id == id).Include(x => x.Community).Include(x => x.ContentPictures).FirstOrDefaultAsync();
            if (Content != null)
            {
        ...
        public async Task UpdateContentAsync(int contentManagementId, ContentManagementDTO content)
        {
            ...
            if (entity != null)
            {
                ...
                {
                    _context.ContentPicture.RemoveRange(_context.ContentPicture.Where(p => p.ContentManagementId == contentManagementId));
                    await _context.SaveChangesAsync();
